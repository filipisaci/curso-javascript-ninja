(function () {
    ...
    */
    ...
    let cpfs = ['049-214 3421-1', '210.458.522-05', '735 500 794 - 22', 
        '101.123-131x32'];
    ...
    let newCpfs = cpfs.map((valor) => {
        return valor.replace(regex, cleanCPF);
    });
    ...
    */
    ...
    function montaCPF(cpf, parte1, parte2, parte3, parte4) {
        return parte1 + '.' + parte2 + '.' + parte3 + '-' + parte4;
    }
    
    let cpfsFormatados = newCpfs.map((cpf) => {
        return cpf.replace(regexCPF, montaCPF);
    });
    
    cpfsFormatados.forEach((cpf) => {
        console.log(cpf);
    });
    ...
    */
    ...
    let regexMes = /ju[ln]ho/g;

    let meses = frase.match(regexMes, '$&');
    ...
    */
    ...
    let codigoHTML = '<DIV><section><blockquote>Texto <img /></blockquote>\
        </section></div>'.toLowerCase();
    let regexHTML = /\<[a-z]+\>/g;
    console.log( codigoHTML.match(regexHTML, '$&') );
    ...
    */
    ...
    codigoHTML = '<DIV><ul><li></li><li></li><li><span></span></li></ul>\
        </div>'.toLowerCase();
    regexHTML = /(\<[a-z]+\>)(\<\/[a-z]+\>)/g;
    console.log( codigoHTML.match(regexHTML, '$&') );
    ...
    */
    ...
    codigoHTML = "<h1>Título da página</h1>\n<p>Este é um parágrafo</p>\n<footer>Rodapé</footer>";
    regexHTML = /(\<[\w]+\>)(.*)(\<\/[\w]+\>)/gm;

    function scrapHTML(bloco, tag1, valor, tag2) {
        return tag1 + 'O texto dentro da tag "' + tag1.replace(/[\<\>]/g,'') +
            '" é "' + valor + '"' + tag2; 
    }
    console.log(codigoHTML.replace(regexHTML, scrapHTML));
})();